...
def train_model():
    ...
    for i in list(numpy.linspace(0.05,0.90,18)):
        for elem in range(num_trials):
            X_train, X_test, y_train, y_test = train_test_split(data.data, data.target, train_size=i)
            model = LogisticRegression(C=10**-10)
            model.fit(X_train,y_train)

            arrar1.append(model.score(X_train,y_train))
            avg_train=numpy.mean(arrar1)

            arrar2.append(model.score(X_test, y_test))
            avg_test=numpy.mean(arrar2)
        train_accuracies.append(avg_train)
        test_accuracies.append(avg_test)
        print("Train accuracy %f"%avg_train)
        print("Test accuracy %f"%avg_test)
